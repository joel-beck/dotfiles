...
From this point on, VS Code is available — switch to it for convenience.
...
### `just install` fails with "already exists but is a regular file"
...
```bash
rm ~/.config/karabiner/karabiner.json && just install
```

Replace the path with whichever file triggered the error.
